...
import { humanizeText, GigaChatError, TableData, ChartData, TableType, generateTable, generateChartData } from "@/lib/gigachat";
  ...
  useEffect(() => {
    ...
    const generateMissingTables = async () => {
      for (const section of sections) {
        ...
        const mentionsTable = section.content && /таблиц[аеиуы]?|table/i.test(section.content);

        if (needsTable && mentionsTable && !alreadyGenerated) {
          try {
            const table = await generateTable(theme, section.title, section.description, 'comparative');
  ...
  useEffect(() => {
    ...
    const ensureCharts = async () => {
      ...
      for (const section of sections) {
        ...
        const mentionsChart = /график|диаграмм|динамик|рисунк/i.test(section.content);

        if ((mentionsChart && needsChart && !alreadyGenerated) || (!hasCharts && needsChart && !alreadyGenerated)) {
          try {
            const chart = await generateChartData(theme, section.title, section.description, 'line');
            if (cancelled) {
              return;
            }
